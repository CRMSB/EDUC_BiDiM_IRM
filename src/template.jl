...
create a new cell and either add :
```julia
md\"\"\"
Your text
\"\"\"
...
or use the : `command/ctrl + M`

Next you can write markdown text.

## Add image

If you want to load image, you can take a look at this [page](https://plutojl.org/en/docs/images/#:~:text=The%20easiest%20way%20to%20add,need%20to%20create%20an%20account.)

The easiest way is to upload them on imgur.com (see links) and then link them with the markdown format :

![](https://i.imgur.com/S9FsjkF.png)

Please keep your image on your computer we will add them at the end somewhere on the github repository
...
## How to call the output of a variable in the text

You can call it with `$myvariable` and you can perform operation directly onto the variable with `$(function_on(myvariable))`
"""

# ╔═╡ 4a4189bd-8d71-4091-b590-5cdb3018c486
a = 1

# ╔═╡ 3768a13e-fc29-425b-8108-706fc1bcad28
md"""
This is the results : a = $a

This is the results  of a + 3: a = $(a + 3)
"""

# ╔═╡ b92a6fc9-0dbe-4f3a-bd8f-e16b940ba5d8
begin
	M = [ 1;2;3;4]
	M2 = [1;2;3;4;; 5;6;7;8]
end

# ╔═╡ 46331e75-b895-44c5-8a8d-5e74dd5c96a8
md"""
## Add equation

You can write latex equation :
...
To help you to show the results of matrix / vector you can use the following package:
```julia
using Latexify
```
this is a test : $M =$ $(latexify_md(M));

and $M2 =$ $(latexify_md(M2))

"""

# ╔═╡ c165f3e2-2874-4330-ae1b-22ec87fc5f1e
md"""
...
Don't change anything in the cell and write your code. If it is a multiline code you need to encapsulate it with :
...
Note that it is possible to use whatever plot backend you want.
In my case I am using CairoMakie it should looks like :

```julia
begin
	f = Figure()
	ax = Axis(f[1,1], title = "my figure")
	scatter!(ax,1:10,1:10)
	f
end
```

the last `f` is required because pluto always plot the output of the last command.

**Note : You can't use the same variable name at different places. The recommended way to work with that limitation is to use function (especially for plots)**
...
Then you can create a second plot in another cell :

```julia
begin
	function myplot2()
		f = Figure()
		ax = Axis(f[1,1], title = "my figure 2")
		lines!(ax,1:10,1:10)
		return f
	end
	myplot2()
...
# ╔═╡ 0bcea9e9-326d-45d8-92f3-177d54fd9a8b
md"""
# Educationnal outputs

At the end of this notebook a Helper functions has been added to help design **Question/Hint/Answer**

Some of them required the package 
```julia
using PlutoTeachingTools
```
In order to add a question :
```julia
md\"\"\"
My question
\"\"\" |> question
...
# ╔═╡ acef4058-7370-4d28-8528-a4f566784e3f
md"""
For hints :
"""
...
# ╔═╡ 3df2208b-d44d-41ad-928d-324c5b15123a
md"""
My question
""" |> question

# ╔═╡ 3db2c9a8-969b-489f-a66e-5d39511ffecc
md"""
test
""" |> hint

# ╔═╡ 1cc9ce5d-8266-4eaa-ab63-4cd334bc94ee
md"""
This is an answer
""" |> answer_folded
